...
- ✅ discarded, because `sim["par"].subconfig("dmrg2")` at the use site is enough:
  promoting the whole `par` tree to tenpy `Config`s in
  `ext_tenpy.TeNPyYAMLSerializer.load`, so that every section reaches the algorithm
  that consumes it with logging and unused-option tracking attached. `load` already
  guarantees the *root* is a `Config`, which is all `subconfig` needs; a section is
  then promoted once, on demand, and stored back into `par`. Revisit only if a
  backend needs a rich `par` that a plain-dict merge cannot produce — three obstacles
  found while trying. (i) The config merge in `Simulation.__init__` grafts raw `cfg`
  subtrees into `par`, so whatever `load` returns is diluted on a fresh run;
  restoring it needs a serializer hook (`adapt`: plain -> rich, applied after the
  patch) — that part works, ~5 lines. (ii) `dictdiffer` then walks a `Config` as a
  plain `MutableMapping`: it reads every option through `__getitem__` (a wall of INFO
  lines, and every option marked used, which *suppresses* genuine unused warnings)
  and `deepcopy`s subtrees it discards, each corpse warning from `Config.__del__`.
  Fixing that needs the inverse hook too (`raw`: rich -> plain, to diff and patch on
  plain data). (iii) Even then, a restart re-promotes the defaults tenpy wrote back
  into `params.yaml`, including sections it never reads when the feature is off
  (`mixer_params`), which warn as unused.
  If revisited, the merge belongs in
